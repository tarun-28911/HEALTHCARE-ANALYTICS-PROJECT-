...
def clean_data(raw):
    ...
    if "Patient_ID" in patients.columns:
        patients["Patient_ID"] = (
            pd.to_numeric(patients["Patient_ID"], errors="coerce")
            .astype("Int64")
            .astype("string")
            .str.strip()
            .str.upper()
    ...
    if "Patient_ID" in visits.columns:
        visits["Patient_ID"] = (
            pd.to_numeric(visits["Patient_ID"], errors="coerce")
            .astype("Int64")
            .astype("string")
            .str.strip()
            .str.upper()
...
# ----------------------------------------------------------------------
# DATA SOURCE — CSV files expected in the same folder as app.py
# ----------------------------------------------------------------------
